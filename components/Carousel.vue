<template>
  <div class = "h-full w-full flex transition duration-75 text-shadow" @mouseover="slideshow = false" @mouseleave="{slideshow = true; t = false}">
    <div class="relative h-full w-full overflow-hidden rounded-3xl">
      ...
      <div class = "absolute max-w-[25%] bottom-10 right-10 text-lg text-white">
        {{ posts[slide].description }}
      </div>
      
      <NuxtLink :to="`/article/${posts[slide].slug}`" class = "text-white absolute text-base hover:scale-110 font-bold bottom-24 w-40 h-14 flex left-[50%] border-2 justify-center items-center -translate-x-[50%] border-white rounded-full"> 
      Läs Mer
      </NuxtLink>

      <nuxt-img src = "arrow.png" @click="slide = (slide + 1) % 3" class = "absolute right-8 top-[50%] -translate-y-[50%] w-10 h-10 hover:scale-110"/>
      <nuxt-img src = "arrow.png" @click=" {if (slide == 0) slide = 2; else slide --;}" class = "absolute left-8 top-[50%] -translate-y-[50%] rotate-180 w-10 h-10 hover:scale-110"/>
